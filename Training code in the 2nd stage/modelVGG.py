...
class ModelVGG(object):
	...
	def train_setup(self):
		...
		label_proc_onehot = prepare_label(self.label_batch, output_size, num_classes=self.conf.num_classes, one_hot=True) # [batch_size, 41, 41]
		...
		raw_gt = tf.reshape(label_proc_onehot, [-1, self.conf.num_classes])
		pgt = tf.gather(raw_gt, indices)

		# Pixel-wise softmax_cross_entropy loss
		loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=prediction, labels=gt) ##GT 的1维的LABEL数据；

		#asloss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=pgt, labels=prediction) ##加了一个对称的LOSS；
		...
		self.train_op = tf.group(train_op_conv, train_op_fc_w, train_op_fc_b) ##对所有的参数进行训练
	...
	def test_setup(self):
		...
		# Create network
		net = DeepLabVGGModel(self.image_batch, num_classes=self.conf.num_classes, is_training=False)
