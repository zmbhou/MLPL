...
# set path for data
data_path = './'
train_lst_path = data_path + 'train_cls.txt'
im_path = data_path + 'JPEGImages/'
sal_path ='path for saliency'
att_path ='path for MLPL'
save_path = './proxy-gt/'
...
def gen_gt(index):
    ...
    for i in range(len(fields) - 1): ##根据类别，读取不同的LABEL；
        k = i + 1
        cls =int(fields[k])
